...
{
    public static class SystemIconsManager
    {
        private static Dictionary<string, Bitmap> m_cachedSmallIcons = new Dictionary<string, Bitmap>();
        private static Dictionary<string, Bitmap> m_cachedLargeIcons = new Dictionary<string, Bitmap>();
        private static Object m_thisLock = new Object();

        public static void AddSmallIconToCache(string extension, byte[] imageContents)
        {
            string ext = extension.ToLower();

            if (!m_cachedSmallIcons.ContainsKey(ext))
                m_cachedSmallIcons.Add(ext, BitmapExtensions.ToBitmap(imageContents));
        }

        public static void AddSmallIconToCache(string extension, Bitmap image)
        {
            string ext = extension.ToLower();

            if (!m_cachedSmallIcons.ContainsKey(ext))
                m_cachedSmallIcons.Add(ext, image);
        }
        ...
        public static void AddLargeIconToCache(string extension, Bitmap image)
        {
            string ext = extension.ToLower();

            if (!m_cachedLargeIcons.ContainsKey(ext))
                m_cachedLargeIcons.Add(ext, image);
        }

        public static Bitmap GetCachedSmallIcon(string extension)
        {
            ...
            try
            {
                smallIcon = m_cachedSmallIcons[ext];
            }
            catch
            {
                lock (m_thisLock)
                {
                    ...
                    try
                    {
                        smallIcon = IconFromExtensionShell(extension, SystemIconsManager.SystemIconSize.Small).ToBitmap();
                    }
                    catch
                    {
                        smallIcon = Orbital7.Extensions.NETFramework.Properties.Resources.Document;
                    }

                    // Add the icon.
                    try
                    {
                        m_cachedSmallIcons.Add(ext, smallIcon);
                    }
                    catch
                    {
                        smallIcon = m_cachedSmallIcons[ext];
        ...
        public static Bitmap GetCachedLargeIcon(string extension)
        {
            ...
            try
            {
                largeIcon = m_cachedLargeIcons[ext];
            }
            catch
            {
                lock (m_thisLock)
                {
                    ...
                    try
                    {
                        largeIcon = IconFromExtensionShell(extension, SystemIconsManager.SystemIconSize.Large).ToBitmap();
                    }
                    catch
                    {
                        largeIcon = Orbital7.Extensions.NETFramework.Properties.Resources.Document;
                    }

                    // Add the icon.
                    try
                    {
                        m_cachedLargeIcons.Add(ext, largeIcon);
                    }
                    catch
                    {
                        largeIcon = m_cachedLargeIcons[ext];
                                                ...
                                                SystemIconSize size)
        {
            ...
            int iconIndex = 0;
            if ((IconPath.Length > 1) && (IconPath[1] != null)) iconIndex = Convert.ToInt32(IconPath[1]);
            IntPtr[] Large = new IntPtr[1], Small = new IntPtr[1];

            //extracts the icon from the file.
            ExtractIconEx(IconPath[0], iconIndex, Large, Small, 1);
